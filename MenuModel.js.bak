...
function normalizeItem(id, raw) {
  ...
  return {
    ...
    sort: (function() {
      if (value.sort === undefined || value.sort === null || value.sort === "") return null
      var n = Number(value.sort)
...
function mergeMenuSources(defaultItems, userItems) {
  ...
  if (!nextItems.root) {
    nextItems.root = { id: "root", parent: "", kind: "menu", icon: "", iconFont: "", label: "Go", title: "", target: "", description: "", aliases: [], when: "", checked: "", action: "", provider: "", sort: null }
